...
resource "aws_sns_topic" "crawler_completion" {
  name = "crawler-completion-${var.environment}"
}
...
resource "aws_cloudwatch_event_rule" "crawler_completion" {
  name        = "crawler-completion"
  description = "Capture Glue crawler completion"

  event_pattern = jsonencode({
    source      = ["aws.glue"]
    detail-type = ["Glue Crawler State Change"]
    detail = {
      crawlerName = [aws_glue_crawler.connection_logs.name]
      state       = ["SUCCEEDED"]
    }
  })
}
...
resource "aws_cloudwatch_event_target" "sns" {
  rule      = aws_cloudwatch_event_rule.crawler_completion.name
  target_id = "SendToSNS"
  arn       = aws_sns_topic.crawler_completion.arn
}

resource "aws_sns_topic_policy" "crawler_completion" {
  arn = aws_sns_topic.crawler_completion.arn
